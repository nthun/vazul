...
test_that("scramble_variables_rowwise works with multiple variable sets", {
    ...
    expect_setequal(c(result$score_a[3], result$score_b[3]), c(70, 80))
...
test_that("scramble_variables_rowwise handles single column sets", {
    ...
    # Single column as string should trigger warning but return unchanged
    expect_warning(
    ...
    # Single column as character vector should also trigger warning
    expect_warning(
...
test_that("scramble_variables_rowwise validates input correctly", {
    ...
    # Test invalid expression (this will fail in tidyselect)
    expect_warning(
...
test_that("scramble_variables_rowwise works with tidyselect expressions", {
    ...
    set.seed(123)
    result1 <- scramble_variables_rowwise(df, starts_with("day_"))

    expect_s3_class(result1, "data.frame")
    expect_equal(names(result1), names(df))
    expect_setequal(c(result1$day_1[1], result1$day_2[1], result1$day_3[1]), c(1, 2, 3))
    expect_setequal(c(result1$day_1[2], result1$day_2[2], result1$day_3[2]), c(4, 5, 6))
    expect_equal(result1$id, df$id)
    expect_equal(result1$score_a, df$score_a)
    expect_equal(result1$score_b, df$score_b)

    # Test with multiple tidyselect expressions
    set.seed(123)
    result2 <- scramble_variables_rowwise(df,
                                          starts_with("day_"),
                                          starts_with("score_")
    )
